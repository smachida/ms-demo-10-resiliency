...
function testCircuitBreaker() {
    ...
    EXEC="docker run --rm -it --network=ms-demo-10-resiliency_my-network alpine"
...
ACCESS_TOKEN=$(curl -k https://writer:secret@$HOST:$PORT/oauth/token -d grant_type=password -d username=vmware -d password=password -s | jq .access_token -r)
...
READER_ACCESS_TOKEN=$(curl -k https://reader:secret@$HOST:$PORT/oauth/token -d grant_type=password -d username=vmware -d password=password -s | jq .access_token -r)
